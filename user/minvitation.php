...
if(count($tid)==1){
    $sqlstmt = mysqli_query($con1,"SELECT * FROM exam_category WHERE id=$tid[0]");
    $datastmt = mysqli_fetch_array($sqlstmt);
    $testname = $datastmt['category'];
}
else{
    $testname="ASSESSMENT";
}
...
foreach($res as $m)
     {
		...
		$mail->isHTML(true);  // Set email format to HTML
			...
			//Change the subject.
// 			$mail->Subject = 'Test Invitation-KnockOnBrain';
			$data = array('tid'=>$tid,'mcqid'=>$mcqid,'cid'=>$cid,'exp'=>$exp,'m1'=>$m['email']);
		    ...
		    $mail->Body='<div class="main-div" style="padding: 1rem; background-color: #ececec;">
                ...
                <img src="https://cdn-icons-png.flaticon.com/128/1581/1581942.png" style="width: 20px;
                            ...
                            text-decoration: none;
                            margin-bottom: -5px !important;"></img>
                <b style="vertical-align: super; text-transform: uppercase;">TEST NAME : '.$testname.'</b>

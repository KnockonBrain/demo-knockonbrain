...
<body>
     <button class="feedback" onclick="location.href = '../user/mcqresult.php';" style="font-size:20px; float: right; margin-top: -100px;">View Results</button>
</body>
...
//fetch id of test
$sql1="SELECT id FROM details WHERE cid=? AND quiz_id=? AND date=?";
$stmt1=$pdo->prepare($sql1);
$stmt1->execute([$cid,$tid,$date]);
...
foreach($res as $m)
     {
		...
		$mail->isHTML(true);  // Set email format to HTML
		    ...
		    $mail->Body='
		      <div class="main-div" style="padding: 1rem; background-color: #ececec;">
                ...
                <center>
                    <div>
                        <a href="'.$result['link']."?cid=".$cid."&id=".$resultid."&tid=".$tid."&m1=".$m['email'].'" style="cursor:pointer !important; background-color: #012970;
                                    border:none;
                                    text-align: center;
                ...
                If the above button does not work for you, attempt the test through the given link: '.$result['link']."?cid=".$cid."&id=".$resultid."&tid=".$tid."&m1=".$m['email'].'
                <br><br>
		...
		//	$mail->Body="Heelooo";
		    ...
		    $mail->AltBody = "Test Link: ".$result['link']."?cid=".$cid."&id=".$resultid."&tid=".$tid."&m1=".$m['email']."  Test email: ".$m['email']."  Company Name: ".$company;
    ...
    echo "<div class='container text-center'><a href='https://phptest.avianglobes.com/user/mcq.php'".$cname."/'><button class='feedback2' style='margin-top:0.6rem'>Schedule More Test</button></a></div>";		

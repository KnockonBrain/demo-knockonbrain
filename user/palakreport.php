<?php
    ...
    $Check = mysqli_query($con,"SELECT * FROM users WHERE ID=$cid");
	$data = mysqli_fetch_array($Check);
	$typec= $data['type'];
	$expdate= $data['exp_date'];
    $todayd = date("Y-m-d H:i:s",time());
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<table style="width: 100%; margin: 5px;">
	...
	<tr>
		...
		<b>Test Ordered: </b>
		<?php
		$mcqnameid=mysqli_query($con,"SELECT * from details where cid=$cid and Email='$var_currentemail' GROUP BY quiz_id order by id desc;");
		$codename= mysqli_query($con,"SELECT * from code_result where cid=$cid and Email='$var_currentemail' GROUP BY lg;");
			$codename1= mysqli_query($con,"SELECT * from code_result where cid=$cid and Email='$var_currentemail' GROUP BY lg;");
		$codedetailsname= mysqli_query($con,"SELECT * from code_details where cid=$cid and email='$var_currentemail'");
		$psychoname = mysqli_query($con,"SELECT * from psycho_details where cid=$cid and email='$var_currentemail'");
		
		$namearr=array();
	    $a=0;
		while($data = mysqli_fetch_array($mcqnameid)){
		        $variable=$data['quiz_id'];
	           	$mcqname=mysqli_query($con,"SELECT * from exam_category where id=$variable");
	           	$dataname = mysqli_fetch_array($mcqname);
	           	$namearr[$a++]=$dataname['category'];
	        }
	   
	  $datay = mysqli_fetch_array($codedetailsname);
	  
	   $arr2=array();
	   $j=0;
	   while($data = mysqli_fetch_array($codename)){
	            $arr2[$j++]=$data['lg']." Coding";
	        }
		$string = join(', ', $namearr);
	        $stringab = join(', ', $arr2);
	        if(!empty($namearr) && !empty($arr2))
	        	echo $string.", ".$stringab;
	        else
	        	echo $string.$stringab;
	  
	  $dataz = mysqli_fetch_array($codename1);
	  if($dataz[0]==NULL){
	      if(!empty($datay[0])){
	          if(!empty($namearr)){
	            echo ", Coding";
	   ...
	   $data=mysqli_fetch_array($psychoname);
		$yesorno=$data['cid'];
		if($yesorno){
			
		if(empty($namearr) and empty($arr2) and empty($datax))
            echo "Psychometric";
        else 
            echo ", Psychometric";}
...
$newrid= $data_attempt1['result_id'];
   
?>
<br>
<b>To view the complete Answer Sheet</b>
<?php
    if($typec!='trial' && $expdate>=$todayd){
        ...
        <?php
         ...
         if($typec=='subscribed'){
             ?>
             <button class="infopt" onclick="mysubscription()">Click Here</button>
             <?php
         }
         else{
             ?>
         
        <button class="infopt" onclick="mytrial()">Click Here</button>
        <?php
         }
        ?>
        <?php
...
<?php
   ...
   {    
       ...
       foreach($testnameIT  as $mcqtestnameindex ) {
           $realid=mysqli_query($con,"SELECT cid from exam_category where category='$mcqtestnameindex'");
           $realdata=mysqli_fetch_array($realid);
           $tally=$realdata[0];
       ?>
...
<table style="width:100%; table-layout: fixed;">
		...
		<td style="width:40%;">
		    ...
		    if($dataCodeA[0]==0)
				echo "Not Attempted";
			     ...
			     $state=mysqli_query($con,"SELECT * FROM code_result WHERE cid=$cid AND email='$var_currentemail' and res_per='$varcodescore' and lg='$datacn' order by rid desc");
                  $data = mysqli_fetch_array($state);
                   if($data['AI_ACCURACY']=="CHEATED")
                  echo "Invalid";
                    else
			...
			if($dataCodeA[0]==0){
			    ...
			    $Duration=mysqli_query($con,"SELECT * FROM code_result WHERE cid=$cid and email='$var_currentemail' and res_per='$varcodescore' and lg='$datacn' order by rid desc");
                $data = mysqli_fetch_array($Duration);
	...
	<?php   
            ...
            ?>
                <div class="progress" style="width: 70%; margin: 3px;">
   			<div class="progress-bar" role="progressbar" style="width:<?php echo $varconvertscore."%"; ?>;" aria-valuenow="25" aria-valuemin="0" aria-valuemax="100"></div>
   			</div>
            <?php 
            }
			else{
...
<b>Important: <i>The following are some behavioral traits of the candidate from his/her responses in the test. However, these need to be validated during the interview seeking examples of instances where the candidate has exhibited these in his/her experience.</i></b><br>
<ul>
    <?php
...
<hr class="solid">
1. This report is based on the candidate’s attempts to the questions and is thus reflective of his performance during the assessment time. These assessment results should always be considered in the context of all available information about a person; do not use this as the sole factor for making Fc employment-related decisions.<br>
2. Response state is generated by our Auto Proctoring Tool: ‘Valid’ be indicated that the response of the candidate to the questions are valid and fair. ‘Invalid’ response state indicates that the inference from the candidate’s response is not valid.<br>
3. Analytical Reasoning: the test of 30 minutes durations with 20 multiple choice questions. These questions are random selected from KOB’s pool of more than 10000 questions categorized into three difficulty levels. The test blue print is of 50% Low, 30% Medium, 20% High difficulty level.<br>
4. Our technical assessments (40+ tests categories that can be selected via drop down menus).<br>
5. KOB’s subject matter experts are constantly updating the question banks keeping them relevant to the current time and need.<br>
6. Coding Assessment: Unique Set of questions and Auto check code on different test cases in background. <br>
7. Psychometric Assessment: We have tie-ups with the experts across who are working with us on personality-based tests. There are 70 set of questions that have a time duration of 30 minutes to attempt. <br><br>
<b>Note: The assessment is the outcome of the candidate’s most recent attempt.</b><br><br>
All the tests are <b>auto proctored</b> using KOB’s Artificial Intelligence based auto proctoring tool that monitors if the candidate is using any unfair means to respond to the questions. We provide a video recording of the test session as per need.
...
<script>
           function mytrial(){
            ...
            text: "You need to buy candidate answersheet addon to unlock this feature!",
           icon: "warning",
            buttons: "subscribe",
        //   buttons: true,
            dangerMode: true,
